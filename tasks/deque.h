...
class Deque {
 private:
  struct Bucket {
    ...
    T* elements_;
    Bucket() : elements_(reinterpret_cast<T*>(new char[size_ * sizeof(T)])) {}
    T& operator[](size_t position) { return elements_[position]; }
  };
...
std::pair<size_t, size_t> Deque<T>::get_rbegin() const {
  if (size_ == 0) {
    return {begin_bucket_ - 1, Bucket::size_ - 1};
  }
  size_t index1 = begin_index_ + size_ - 1;
  size_t index2 = begin_bucket_ + index1 / Bucket::size_;
  index1 %= Bucket::size_;
...
Deque<T>::Deque(size_t sz)
    : size_(sz),
      begin_bucket_(1),
      begin_index_(0),
      bucket_quantity_((sz + Bucket::size_ - 1) / Bucket::size_ + 2) {
  if (size_ == 0) {
    return;
  }
  buckets_ = new Bucket[bucket_quantity_];
...
Deque<T>::base_iterator<is_constant>::operator--() {
  if (position_ == 0) {
    --bucket_;
    position_ = Bucket::size_ - 1;
    pointer_ = bucket_->elements_ + Bucket::size_ - 1;
  } else {
    --position_;
    --pointer_;
  }
...
Deque<T>::base_iterator<is_constant>::operator+=(int shift) {
  ...
  new_position += shift;
  bucket_ += new_position / (difference_type)Bucket::size_;
  new_position %= (difference_type)Bucket::size_;
  if (new_position < 0) {
    --bucket_;
    new_position += Bucket::size_;
  }
...
Deque<T>::base_iterator<is_constant>::operator-(
  ...
  difference_type answer = (bucket_ - other.bucket_) * Bucket::size_ +
...
void Deque<T>::push_back(const T& value) {
  auto pos = get_rbegin();
  ++pos.second;
  if (pos.second == Bucket::size_) {
    pos.second -= Bucket::size_;
    pos.first += 1;
  }
  if (pos.first == bucket_quantity_ - 1 && pos.second == Bucket::size_ - 1) {
...
void Deque<T>::push_front(const T& value) {
  if (begin_bucket_ == 1 && begin_index_ == 0) {
    ...
    try {
      ...
      new (new_buckets[bucket_quantity_].elements_ + Bucket::size_ - 1) T(value);
    } catch (...) {
      delete[] new_buckets;
      throw;
    }
...
T& Deque<T>::operator[](size_t position) {
  position += begin_bucket_ * Bucket::size_ + begin_index_;
